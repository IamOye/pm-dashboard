...
function initDB() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS projects (
      ...
      pmbok_stage   INTEGER DEFAULT 0,
      pmbok_complete INTEGER DEFAULT 0,
      created_at    TEXT DEFAULT (datetime('now')),
      updated_at    TEXT DEFAULT (datetime('now'))
    );

    -- ─────────────────────────────────────────────
    -- PMBOK WIZARD STATE (per project, per stage)
    -- ─────────────────────────────────────────────
    CREATE TABLE IF NOT EXISTS pmbok_stages (
      id          TEXT PRIMARY KEY,
    ...
    CREATE TABLE IF NOT EXISTS stakeholders (
      ...
      email       TEXT,
      phone       TEXT,
      power       TEXT DEFAULT 'Low',
      interest    TEXT DEFAULT 'Low',
      engagement  TEXT DEFAULT 'Neutral',
      notes       TEXT,
      FOREIGN KEY (project_id) REFERENCES projects(id) ON DELETE CASCADE
    );
    ...
    CREATE TABLE IF NOT EXISTS budget_items (
      ...
      variance    REAL GENERATED ALWAYS AS (estimated - actual) VIRTUAL,
      FOREIGN KEY (project_id) REFERENCES projects(id) ON DELETE CASCADE
    );
    ...
    -- COMMUNICATIONS PLAN
    -- ─────────────────────────────────────────────
    CREATE TABLE IF NOT EXISTS comms_plan (
      id              TEXT PRIMARY KEY,
      project_id      TEXT NOT NULL,
      stakeholder_id  TEXT,
